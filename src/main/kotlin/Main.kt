 ...
 */
const val MEMORY_SIZE = 0x1000u
const val MEMORY_PROGRAM_START = 0x200u
const val MEMORY_FONTSET_START = 0x000u
const val FONT_SPRITE_SIZE = 0x5u
...
class Display {
    ...
    private val bufferSize = (WIDTH / 8u * HEIGHT).toInt()
    private val buffer = UByteArray((WIDTH / 8u * HEIGHT).toInt())

    fun clear() = repeat(bufferSize) { buffer.set(it, 0u) }

    fun draw(vx: UByte, vy: UByte, sprites: List<UByte>): Boolean {
        val xStart = vx % WIDTH
        val yStart = vy % HEIGHT
        val (index, xOffset) = getBufferIndexAndXOffset(xStart, yStart)
        var collision = false
        ...
        for (y in yStart until minOf(HEIGHT, yStart + sprites.size.toUInt())) {
            ...
            if (xOffset == 0) {
                ...
                if (index % (WIDTH / 8u).toInt() < (WIDTH / 8u - 1u).toInt()) {
                    buffer[index + 1] = applyXOR(buffer[index + 1], (sprite.toUInt() shl (8 - xOffset)).toUByte())
                }
            }
        }
        return collision
